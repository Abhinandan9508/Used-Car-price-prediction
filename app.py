...
# --------------------------------------------------
# Load Dataset (PROJECT ROOT = D:\New folder)
# --------------------------------------------------
...
models = sorted(df[df["brand"] == brand]["model"].dropna().astype(str).unique())
model = st.sidebar.selectbox("Model", models)

fuel = st.sidebar.selectbox("Fuel Type", sorted(df["fuel_type"].dropna().astype(str).unique()))
trans = st.sidebar.selectbox("Transmission", sorted(df["transmission_type"].dropna().astype(str).unique()))
city = st.sidebar.selectbox("City", sorted(df["city"].dropna().astype(str).unique()))
owners = st.sidebar.selectbox("Number of Owners", sorted(df["number_of_owners"].dropna().astype(int).unique()))

# --------------------------------------------------
# Auto Bodytype
# --------------------------------------------------
bodytype_series = df[(df["brand"] == brand) & (df["model"] == model)]["bodytype"].dropna()
body = str(bodytype_series.mode()[0]) if len(bodytype_series) > 0 else "Unknown"
...
# --------------------------------------------------
st.markdown("---")
st.markdown("<center>Developed by <b>Team Plan B</b> | Machine Learning Project</center>", unsafe_allow_html=True)
